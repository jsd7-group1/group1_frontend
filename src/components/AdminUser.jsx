      ...
      <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-6">
        {users.map(user => (
          <div key={user._id} className="bg-white p-4 rounded-lg shadow-md">
            ...
            {user.imageUrl ? (
              <img src={user.imageUrl} alt={user.fullname} className="w-full h-48 object-cover mb-4 rounded-lg" />
            ) : (
              <h1 className="text-gray-700">No Image</h1>
            )}
            <p className="text-gray-700"><strong>ID</strong>: {user._id}</p>
            <p className="text-gray-700"><strong>Email</strong>: {user.email}</p>
            <p className="text-gray-700"><strong>FullName:</strong> {user.fullName}</p>
          </div>
        ))}
      </div>
    </div>
